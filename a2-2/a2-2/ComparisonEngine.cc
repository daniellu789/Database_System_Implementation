...
RecordComparison :: RecordComparison(OrderMaker *order){
	this->order=order;
}

bool RecordComparison::operator ()( Record* left, Record* right) const{
	//cout<<"inside RecordComparison::operator ()"<<endl;
	ComparisonEngine ceng;

	if (ceng.Compare (left, right, order) < 0) // compare returns 0 is left is less than right
		return true;
	else
		return false;
}

RunRecordComparison :: RunRecordComparison(OrderMaker *order){
	this->order=order;
}

bool RunRecordComparison::operator ()( RunRecord* left, RunRecord* right) const{
	//cout<<"inside RecordComparison::operator ()"<<endl;
	ComparisonEngine ceng;


	if (ceng.Compare (left->record, right->record, order) >= 0) // compare returns 0 is left is less than right
		return true;
	else
		return false;
}
