...
public:
	...
	// like the last one, but for eunary operations
...
public:
	RecordComparison(OrderMaker *order);

	//compare two records to put them in the priority queue
	bool operator() ( Record *left, Record  *right) const;
};

//class to provide the compare function for the STL based priority queue implementation for executing the external merge function of the runs
class RunRecordComparison {

private:
	OrderMaker *order;

public:
	RunRecordComparison(OrderMaker *order);

	//compare two records contained in the RunRecord pointers 
