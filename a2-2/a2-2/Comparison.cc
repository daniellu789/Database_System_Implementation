...
OrderMaker :: OrderMaker(int numAttrs, int* attrs, Type* types){
	numAtts=numAttrs;
...
void OrderMaker :: WriteInFileMeta (std::ofstream& FileMeta) {
	
	FileMeta<<numAtts<<endl;

	for (int i = 0; i < numAtts; i++)
	{FileMeta<<whichAtts[i]<<"|";
		if (whichTypes[i] == Int)
			FileMeta<<"Int"<<endl;
		else if (whichTypes[i] == Double)
			FileMeta<<"Double"<<endl;
		else
			FileMeta<<"String"<<endl;
	}
}
